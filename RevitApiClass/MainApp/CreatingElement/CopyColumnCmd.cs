...
namespace MainApp.CreatingElement
{
   ...
   internal class CopyColumnCmd : ExternalCommand
   {
      public override void Execute()
      {
         using (var tx= new Transaction(Document,"Move an element"))
         {
            ...
            CopyColumn(Document, column);

            tx.Commit();
         }
      }

      public void CopyColumn(Document document, FamilyInstance column)
      {
         ...
         ElementTransformUtils.CopyElement(document, column.Id, newPlace -oldPlace);
      }
